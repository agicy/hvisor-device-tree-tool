...
use nom::character::complete::{char, digit1, line_ending, multispace0, space1};
use nom::combinator::{map, map_res, opt, peek, recognize};
use nom::sequence::{delimited, preceded};

use super::parser::escape_c_string;
use super::{byte_offset_to_line_col, line_to_byte_offset};
use crate::dts::ParseError;
...
fn parse_linemarker(input: &[u8]) -> IResult<&[u8], Linemarker> {
    ...
    let (input, line) = map_res(map_res(digit1, str::from_utf8), usize::from_str)(input)?;
    let (input, _) = space1(input)?;
    let (input, path) =
        delimited(char('"'), map(escape_c_string, PathBuf::from), char('"'))(input)?;
    let (input, flag) = opt(preceded(
        space1,
...
fn find_linemarker_start(input: &[u8]) -> IResult<&[u8], &[u8]> {
    ...
    // Since nom doesn't have `find_substring` in public API in same way, we can use simple slice search or `take_until`

    // Simplification: search for first occurrence of '#'
    let (rem, pre) = take_until("#")(input)?;
    Ok((rem, pre))
}
...
fn parse_linemarkers(
    ...
    while let Ok((rem, (pre, marker))) = find_linemarker(buf) {
...
fn parse_include(input: &[u8]) -> IResult<&[u8], String> {
    preceded(
        tag("/include/"),
        preceded(
            multispace0, // changed from multispace1 to multispace0 to be safe, usually space is required but check usage
    ...
    for (index, win) in buf.windows("/include/".len()).enumerate() {
        if win == b"/include/" {
            match parse_include(&buf[index..]) {
                Ok((rem, file)) => return Some((&buf[..index], PathBuf::from(file), rem)),
                Err(_) => {}
            }
        }
    }
...
pub fn include_files<P: AsRef<Path>, I: AsRef<Path>>(
    ...
    fn _include_files(
        ...
        fn first_linemarker(input: &[u8]) -> IResult<&[u8], (&[u8], Linemarker)> {
            let (input, marker) = peek(parse_linemarker)(input)?;
            let (input, line) = recognize(parse_linemarker)(input)?;
            Ok((input, (line, marker)))
        }

        let start_bound = if let Ok((rem, (line, marker))) = first_linemarker(buf) {
